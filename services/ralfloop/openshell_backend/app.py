...
BASE_DIR = Path("${HOME}/ralfloop_agent_scaffold/.openshell_backend")
...
def _mediaset_call_playback_check_from_probe(probe_data: dict) -> dict:
    ...
    if not (_sid and _raw):
        try:
            import json as _json
            _ch_file = "${HOME}/gatto/cat/plugins/stream_scraper/channels_out.json"
    ...
    if (
        ...
        if not data.get("ok"):
            try:
                ...
                cat_root = _Path("${HOME}/gatto")
                plugin_root = _Path("${HOME}/gatto/cat/plugins/stream_scraper")
        ...
        if not data.get("ok") and not probe_data.get("ok"):
            try:
                ch_file = "${HOME}/gatto/cat/plugins/stream_scraper/channels_out.json"
